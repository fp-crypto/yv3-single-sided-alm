...
contract Strategy is BaseStrategy, IUniswapV3SwapCallback {
    ...
    constructor(address _asset, string memory _name, address _steerLP) BaseStrategy(_asset, _name) {
        STEER_LP = ISushiMultiPositionLiquidityManager(_steerLP);
        _POOL = ISushiMultiPositionLiquidityManager(_steerLP).pool();
        address _token0 = ISushiMultiPositionLiquidityManager(_steerLP).token0();
        if (address(asset) == _token0) {
            _ASSET_IS_TOKEN_0 = true;
            _OTHER_TOKEN = ISushiMultiPositionLiquidityManager(_steerLP).token1();
        } else {
    ...
    function lpVaultInAsset() public view returns (uint256 valueLpInAssetTerms) {
        ...
        if (_ASSET_IS_TOKEN_0) {
            uint256 valueMyToken1InAssetTerms =
                FullMath.mulDiv(FullMath.mulDiv(myHoldingsToken1, sqrtPriceX96, Q96), sqrtPriceX96, Q96);
            valueLpInAssetTerms = myHoldingsToken0 + valueMyToken1InAssetTerms;
        } else {
            uint256 valueMyToken0InAssetTerms =
                FullMath.mulDiv(FullMath.mulDiv(myHoldingsToken0, Q96, sqrtPriceX96), Q96, sqrtPriceX96);
        ...
        if (total0InLp == 0 && total1InLp == 0) {
            ...
            if (_ASSET_IS_TOKEN_0) {
                ...
                valueLpHoldingOfOtherToken_inAssetTokenTerms =
                    FullMath.mulDiv(FullMath.mulDiv(total1InLp, sqrtPriceX96, Q96), sqrtPriceX96, Q96);
                ...
                valueLpHoldingOfOtherToken_inAssetTokenTerms =
                    FullMath.mulDiv(FullMath.mulDiv(total0InLp, Q96, sqrtPriceX96), Q96, sqrtPriceX96);
            ...
            if (totalLpValue_inAssetTokenTerms == 0) {
                ...
                amountToSwap = FullMath.mulDiv(
                    assetBalance, valueLpHoldingOfOtherToken_inAssetTokenTerms, totalLpValue_inAssetTokenTerms
                );
            }
        }
    }
    ...
    function _depositInLp() internal {
        ...
        uint256 amountToSwap = _calculateAmountToSwapForDeposit(assetBalance, total0InLp, total1InLp, sqrtPriceX96);
    ...
    function _withdrawFromLp(uint256 _amountAssetToWithdraw) internal {
        ...
        uint256 otherTokenReceivedFromLp =
            ERC20(_OTHER_TOKEN).balanceOf(address(this)) - otherTokenBalanceBeforeWithdraw;
